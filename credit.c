...
void card_id_checker(long card_id);
void verify_card_checker(long card_verify, int which);
...
long get_long(const char *format, ...)
{
    ...
    do
    {
        ...
        if (errno == ERANGE || long_value < LONG_MIN || long_value >= LONG_MAX || *endptr != '\n')
        {
...
void    verify_card_checker(long card_verify, int which)
{
    ...
    int digit_1 = card_number % 10;
    long mem1 = card_number / 10;
    int digit_2 = mem1 % 10;
    long mem2 = mem1 / 10;
    int digit_3 = mem2 % 10;
    long mem3 = mem2 / 10;
